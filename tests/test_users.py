...
class TestUser(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.current_user_tokens = self.client().post('/api/v1/login', json={
               'phone_no': '+254700000000', 'password': '1Admin234'
               })
        self.auth_header = {'Authorization': f'Bearer {self.current_user_tokens.json["access_token"]}'}

        with self.app.app_context():
            db.create_all()

    def test_user_creation(self):
        res = self.client().post('/api/v1/users', json=self.user_data, headers=self.auth_header)
    ...
    def test_user_update(self):
        ...
        res = self.client().put('/api/v1/users/1', json={"middle_name":"Test_user_updated"}, headers=self.auth_header)
        self.assertEqual(res.status_code, 201)
        res = self.client().put('/api/v1/users/1', json={"first_name":"Test_user_updated"}, headers=self.auth_header)
        self.assertEqual(res.status_code, 201)
        self.assertIn('Test_user_updated', str(res.data))

    def test_user_update_self(self):
        """Test API can update an existing user. (PUT request)"""
        res = self.client().put('/api/v1/users/me', json={"middle_name":"Test_user_updated"}, headers=self.auth_header)
        self.assertEqual(res.status_code, 201)
        res = self.client().put('/api/v1/users/me', json={"first_name":"Test_user_updated"}, headers=self.auth_header)
        self.assertEqual(res.status_code, 201)
        self.assertIn('Test_user_updated', str(res.data))
    ...
    def test_user_deletion(self):
        ...
        with self.app.app_context():
            ...
            if res.status_code == 201:
                ...
                db.session.commit()
                res = self.client().delete(f'/api/v1/users/{user_to_delete.id}', headers=self.auth_header)
    ...
    def test_get_user_by_phone(self):
        ...
        res = self.client().get('/api/v1/users/phone_no/+254700000000', headers=self.auth_header)
        self.assertEqual(res.status_code, 200)
        self.assertIn('Root Admin', str(res.data))
        self.assertTrue(res.json['first_name'] == 'Root Admin')
        res = self.client().get('api/v1/user/phone/+254700347287', headers=self.auth_header)
