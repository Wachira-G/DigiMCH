...
class TestAuth(TestCase):
    ...
    def test_user_login(self):
        response = self.client.post('/api/v1/login', json={
               'phone_no': self.admins_phone, 'password': self.admins_passwd
               })
        self.assertEqual(response.status_code, 200)
        self.assertIn('access_token', response.json)
        self.assertIn('refresh_token', response.json)
        self.assertNotEqual(response.json['access_token'], response.json['refresh_token'])
    ...
    def get_access_token_for_user(self, phone_no, password):
        ...
        response = self.client.post('/api/v1/login', json={'phone_no': phone_no, 'password': password})
    ...
    def get_user_tokens(self, phone_no, password)-> dict:
        ...
        password=self.admins_passwd
        response = self.client.post('/api/v1/login', json={'phone_no': phone_no, 'password': password})
        return {"access_token": response.json.get('access_token'), "refresh_token": response.json.get('refresh_token')}
