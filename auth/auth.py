...
def validate_phone_passwd(data) -> tuple:
    ...
    if not phone_no or not is_valid_kenyan_phone(phone_no):
        return False, jsonify({'message': 'Please provide a valid Kenyan phone number'}), 400
...
def login_post():
    ...
    if not valid:
        return phone_no, password  # phone_no and password are actually the error response and status code

    success, user_or_patient_instance = get_user_or_patient(phone_no)
    if not success:
        return user_or_patient_instance  # user_or_patient_instance is actually the error response
...
def reset_password():
    ...
    if not code:
        return jsonify({'message': 'Please provide the code sent to your phone number'}), 400

    if not password or not is_valid_password(password):
        return jsonify({'message': 'Please provide valid password'}), 400

    if not password_confirm or not is_valid_password(password_confirm) or password != password_confirm:
...
def change_phone_number():
    ...
    if new_phone_no == phone_no:
        return jsonify({'message': 'New phone number cannot be the same as the old one'}), 400
